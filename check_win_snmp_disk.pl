...
foreach $LDISK (@LDISKS){
	...
	if ( $PERCENTUSED < $WARN ) { 
		#DISK C: OK (Used: 83.5%) Total: 49.9 GB Used: 41.7GB Free: 8.2GB
		$LONGOUTPUT.="\nDISK $DISKNAME: OK \(Used: $PERCENTUSED%\) Total: $TOTAL$TOTALUNITS Used: $USED$TOTALUNITS Free: $FREE$TOTALUNITS";
	...
	if ( $PERCENTUSED < $CRITICAL ) { 
		$LONGOUTPUT.="\nDISK $DISKNAME: WARNING \(Used: $PERCENTUSED%\) Total: $TOTAL$TOTALUNITS Used: $USED$TOTALUNITS Free: $FREE$TOTALUNITS"; 
		$PERFOUTPUT.="\'Disk $DISKNAME\'=$USED$TOTALUNITS;$WARNSIZE;$CRITSIZE;0;$TOTAL \'Disk $DISKNAME\'=$PERCENTUSED%;$WARN;$CRITICAL ";
	...
	if ( $PERCENTUSED >= $CRITICAL ) {
		$LONGOUTPUT.="\nDISK $DISKNAME: CRITICAL \(Used: $PERCENTUSED%\) Total: $TOTAL$TOTALUNITS Used: $USED$TOTALUNITS Free: $FREE$TOTALUNITS";
		$PERFOUTPUT.="\'Disk $DISKNAME\'=$USED$TOTALUNITS;$WARNSIZE;$CRITSIZE;0;$TOTAL \'Disk $DISKNAME\'=$PERCENTUSED%;$WARN;$CRITICAL ";
...
print "$ERROUTPUT $OUTPUT";
print "$LONGOUTPUT|$PERFOUTPUT\n";
exit $EXIT_CODE;
