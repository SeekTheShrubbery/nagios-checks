...
foreach $LDISK (@LDISKS){
	...
	SWITCH: {
		if ($RAWSIZE * $VALUE > $MNORMSIZE) {
			...
			$WARNRAWSIZE = nearest(.01, $RAWSIZE*(getnewlevel($RAWSIZE*$VALUE, $WARN/100)));
			$CRITRAWSIZE = nearest(.01, $RAWSIZE*(getnewlevel($RAWSIZE*$VALUE, $CRITICAL/100))); last SWITCH;
		};	
		if ($RAWSIZE > 0) {
			$PERCENTUSED = nearest(.01, $RAWUSED/$RAWSIZE * 100);
			$WARNRAWSIZE = nearest(.01, $RAWSIZE/100*$WARN);
			$CRITRAWSIZE = nearest(.01, $RAWSIZE/100*$CRITICAL); last SWITCH;
		};
		$PERCENTUSED = 0;
		$WARNRAWSIZE = 0;
		$CRITRAWSIZE = 0;
	...
	if ( $PERCENTUSED < $WARN ) { 
		...
		$PERFOUTPUT.="\"Disk $DISKNAME\"=$USED$TOTALUNITS;$WARNSIZE;$CRITSIZE;0;$TOTAL ";
		$OUTPUT.="DISK $DISKNAME - OK, ";
		if ( ! $EXIT_CODE ){
